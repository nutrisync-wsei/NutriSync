import AboutIcon from '@/assets/svg/About.svg';
import ArrowLeftIcon from '@/assets/svg/ArrowLeft.svg';
import ChevronRightIcon from '@/assets/svg/ChevronRight.svg';
import DisabledIcon from '@/assets/svg/Disabled.svg';
import ErrorIcon from '@/assets/svg/Error.svg';
import FacebookIcon from '@/assets/svg/Facebook.svg';
import FridgeIcon from '@/assets/svg/Fridge.svg';
import GithubIcon from '@/assets/svg/Github.svg';
import HomeIcon from '@/assets/svg/Home.svg';
import LogoutIcon from '@/assets/svg/Logout.svg';
import NeedleTriangleIcon from '@/assets/svg/NeedleTriangle.svg';
import NotificationsIcon from '@/assets/svg/Notification.svg';
import ProfileIcon from '@/assets/svg/Profile.svg';
import ScaleIcon from '@/assets/svg/Scale.svg';
import SettingsProfileIcon from '@/assets/svg/SettingsProfile.svg';
import SpotifyIcon from '@/assets/svg/Spotify.svg';
import StoveIcon from '@/assets/svg/Stove.svg';
import SubscriptionIcon from '@/assets/svg/Subscription.svg';
import SuccessIcon from '@/assets/svg/Success.svg';
import SupportIcon from '@/assets/svg/Support.svg';

const Icon = {
  Error: ErrorIcon,
  Success: SuccessIcon,
  Disabled: DisabledIcon,
  Facebook: FacebookIcon,
  Github: GithubIcon,
  Spotify: SpotifyIcon,
  NeedleTriangle: NeedleTriangleIcon,
  Fridge: FridgeIcon,
  Home: HomeIcon,
  Profile: ProfileIcon,
  Scale: ScaleIcon,
  Stove: StoveIcon,
  About: AboutIcon,
  ChevronRight: ChevronRightIcon,
  Logout: LogoutIcon,
  Notifications: NotificationsIcon,
  Subscription: SubscriptionIcon,
  Support: SupportIcon,
  SettingsProfile: SettingsProfileIcon,
  ArrowLeft: ArrowLeftIcon,
};

export default Icon;
